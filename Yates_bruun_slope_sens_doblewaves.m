...
addpath(genpath('/home/isabel/Escritorio/PhD_UoA/Shoreshop_data/'));
...
tanb = 0.0003; %slope at Miami or similar Anthanasiou (2019) <0.001
fprintf('Beach slope %d .\n',tanb);
tanb2= 0.0001;
...
subplot(2,1,2);
plot(t(1:8*15:end),Y(1:8*15:end), t(1:8*15:end),Y_b(1:8*15:end),'r'); 
ytxt = {'$Y_{Yates+Bruun} [m]$'};
ylabel(ytxt,'interpreter','latex','FontSize',15);
axis([min(t) max(t) -60 10]); datetick('x','keeplimits');
legend('Yates','Yates+Bruun','Location','southwest')

%% Beach Slope sensibility
